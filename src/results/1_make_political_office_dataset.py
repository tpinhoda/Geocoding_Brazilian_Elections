...
import logging
import pandas as pd
from os import listdir, environ, remove, mkdir
...
def structure_data(office, input_path, output_path):
    ...
    for filename in tqdm(filenames, leave=False):
        ...
        raw_data = pd.read_csv(
            filepath, sep=";", encoding="latin1", na_values=["#NULO#", -1, -3]
        )
        ...
        filtered_raw_data.drop_duplicates(
            subset=["NM_MUNICIPIO", "NR_ZONA", "NR_SECAO"], inplace=True
        )
        # Index filtered raw data
        filtered_raw_data.set_index(
            ["NM_MUNICIPIO", "NR_ZONA", "NR_SECAO", "NR_LOCAL_VOTACAO"], inplace=True
        )
        ...
        structured_data.to_csv(join(output_path, filename), index=False)
