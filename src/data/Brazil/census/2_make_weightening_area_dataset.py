...
def explode(indf):
    outdf = geopd.GeoDataFrame(columns=indf.columns)
    for idx, row in indf.iterrows():
        if type(row.geometry) == Polygon:
            outdf = outdf.append(row,ignore_index=True)
        if type(row.geometry) == MultiPolygon:
            multdf = geopd.GeoDataFrame(columns=indf.columns)
...
def disolve_digital_mesh(digital_mesh_filepath, output_digital_mesh, attr):
    ...
    map_func = {'CD_GEOCODM': 'first',
                'NM_MUNICIP': 'first',
                'geometry': lambda x: x.values.tolist()}

    data_aggr = digital_mesh.dissolve(by=attr, aggfunc='first', as_index=False)
    data_aggr = explode(data_aggr)
...
if __name__ == '__main__':
    ...
    digital_mesh_filepath = project_dir + '/data/interim/{}/census_data/{}/census_tract/digital_mesh/{}.geojson'.format(country, census_year, state)
    input_filepath = project_dir + '/data/interim/{}/census_data/{}/census_tract/universal_results/states/{}/'.format(country, census_year,state)
    output_filepath = project_dir + '/data/interim/{}/census_data/{}/weightening_area/universal_results/states/{}/'.format(country, census_year, state)
    output_digital_mesh = project_dir + '/data/interim/{}/census_data/{}/weightening_area/digital_mesh/{}.geojson'.format(country, census_year, state)
