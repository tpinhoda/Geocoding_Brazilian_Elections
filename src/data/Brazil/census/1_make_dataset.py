...
def structure_data(input_filepath, reference_filepath, digital_mesh_filepath ,output_filepath, output_digital_mesh):
    ...
    for filename in tqdm(filenames, leave=False):
        ...
        structured_data.to_csv(output_filepath+filename+'.csv', index=False, sep=';')            
...
if __name__ == '__main__':
    ...
    #Set paths
    input_filepath = project_dir + '/data/raw/{}/census_data/{}/census_tract/universal_results/{}/'.format(country, census_year, state)
    reference_filepath = project_dir + '/data/raw/{}/census_data/{}/weightening_area/weightening_area_by_census_tract.csv'.format(country, census_year, state)
    digital_mesh_filepath = project_dir + '/data/raw/{}/census_data/{}/census_tract/digital_mesh/{}/43SEE250GC_SIR.shp'.format(country, census_year, state)
    
    output_filepath = project_dir + '/data/interim/{}/census_data/{}/census_tract/universal_results/states/{}/'.format(country, census_year,state)
    output_digital_mesh = project_dir + '/data/interim/{}/census_data/{}/census_tract/digital_mesh/{}.geojson'.format(country, census_year, state)
    ...
    structure_data(input_filepath, reference_filepath , digital_mesh_filepath, output_filepath, output_digital_mesh)
