...
def generate_city_limits_measure(df_polling_places, df_cities_maps, buffer_values):
    ...
    geometry = [
        Point((row.lon, row.lat)) for index, row in df_polling_places.iterrows()
    ]
    ...
    to_concat = []
    for index, location in tqdm(
        df_polling_places.iterrows(), total=df_polling_places.shape[0], leave=False
    ):
        # Get the city map
...
def generate_levenshtein_measure(df_polling_places):
    ...
    df_polling_places["lev_dist"] = df_polling_places.progress_apply(
        lambda x: Levenshtein.ratio(
            x["query_address"].lower(), x["fetched_address"].lower()
        ),
        axis=1,
    )
...
def generate_rural_areas_mark(df_polling_places):
    ...
    tqdm.pandas(leave=False)
    df_polling_places["rural"] = df_polling_places["query_address"].progress_apply(
        lambda x: True if any(i in x.lower() for i in searchfor) else False
    )
...
if __name__ == "__main__":
    ...
    df_polling_places = generate_city_limits_measure(
        df_polling_places, df_cities_maps, buffer_values=buffers
    )
